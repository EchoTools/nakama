...
func TestLatencyHistory_writeWithRetry_BoundedAttempts(t *testing.T) {
	...
	// letting the loop run until it exhausts its bounded attempts.
	...
	if nk.writeAttempts > latencyRetryMaxAttempts {
		t.Errorf("attempts %d exceeded bound %d", nk.writeAttempts, latencyRetryMaxAttempts)
	}
}
