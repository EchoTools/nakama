...
// messages. It gates message dispatch in the IncomingLoop above (SEC-2):
// without it a single established WebSocket can submit messages as fast as the
// TCP window allows, turning any cheap-request/expensive-handler pair — e.g.
// SEC-1's ConfigRequest -> storage read — into a sustained database-exhaustion
// DoS. Connection-level rate limits do not throttle messages within an already-
// established socket, so the per-session cap lives here alongside the loop that
// enforces it.
//
