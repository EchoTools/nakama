...
func (d *DiscordAppBot) handleAuthenticationConflict(ctx context.Context, logger runtime.Logger, discordID string, username string) (string, error) {
	...
	logger.WithFields(map[string]interface{}{
		...
		"username":   username,
	}).Info("Handling authentication conflict: attempting to recover from username conflict")
	...
	if err != nil {
		logger.WithFields(map[string]interface{}{
			...
			"username":   username,
			"error":      err,
		}).Error("Failed to lookup account by username during conflict recovery")
	...
	}).Info("Found conflicting account - trying recovery")
	...
	if err != nil {
		logger.WithFields(map[string]interface{}{
			...
			"conflicting_user_id": conflictingUserID,
			"error":               err,
		}).Error("Failed to get account details during recovery")
	...
	}).Info("Retrieved account details")
	...
	if err := d.linkCustomID(ctx, logger, conflictingUserID, discordID); err != nil {
		logger.WithFields(map[string]interface{}{
			...
			"conflicting_user_id": conflictingUserID,
			"error":               err,
		}).Error("Failed to link custom ID during recovery")
			...
			if err != nil {
				...
				if d.isAlreadyExistsError(err) && strings.Contains(err.Error(), "Username is already in use") {
					logger.WithFields(map[string]interface{}{
						"discord_id": discordID,
						"username":   username,
					}).Info("Username conflict - trying to recover")
