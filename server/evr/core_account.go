...
func (c *ClientProfile) SetDefaults() error {
	...
	if c.CombatWeapon == "" {
		c.CombatWeapon = "assault"
	}
	if c.CombatGrenade == "" {
		c.CombatGrenade = "arc"
	}
	if c.CombatDominantHand == 0 {
		c.CombatDominantHand = 1
	}
	if c.CombatAbility == "" {
		c.CombatAbility = "buff"
	}
...
func (s *ServerProfile) SetDefaults() error {
	s.PurchasedCombat = 1
	s.DeveloperFeatures = nil
	s.EquippedCosmetics.Instances.Unified.Slots.SecondEmote = s.EquippedCosmetics.Instances.Unified.Slots.Emote
	s.MaybeStale = nil
	...
	v := reflect.ValueOf(&s.UnlockedCosmetics.Arena).Elem()
	for i := 0; i < v.NumField(); i++ {
		// Set all the restricted and blocked fields to false
		tag := v.Type().Field(i).Tag.Get("validate")
		if strings.Contains(tag, "restricted") || strings.Contains(tag, "blocked") {
			if v.Field(i).CanSet() {
...
type ArenaUnlocks struct {
	...
	*/
	...
	RWDTintS1BDefault           bool `json:"rwd_tint_s1_b_default,omitempty"`
	...
	TintBlueIDefault            bool `json:"tint_blue_i_default,omitempty"`
	...
	TintNeutralSummerADefault   bool `json:"tint_neutral_summer_a_default,omitempty"`
	...
	TintOrangeJDefault          bool `json:"tint_orange_j_default,omitempty"`
	...
	TintS1DDefault              bool `json:"rwd_tint_s1_d_default,omitempty"`
	...
	TintS3TintB                 bool `json:"rwd_tint_s3_tint_b,omitempty"`
...
func (a *ArenaUnlocks) String() string {
	return "ArenaUnlocks()"
}

type CombatUnlocks struct {
	// TODO FIXME is this really all there is?
	DecalCombatFlamingoA bool `json:"decal_combat_flamingo_a,omitempty"`
	DecalCombatLogoA     bool `json:"decal_combat_logo_a,omitempty"`
	EmoteDizzyEyesA      bool `json:"emote_dizzy_eyes_a,omitempty"`
	PatternLightningA    bool `json:"pattern_lightning_a,omitempty"`
...
// TODO FIXME Put this into a separate registry for handling profiles.
// Create a lookup table for the arena unlocks in the form of map[string]FieldName using reflect
func createArenaUnlocksFieldByKey() map[string]string {
	arenaUnlocks := make(map[string]string)
	arenaType := reflect.TypeOf(ArenaUnlocks{})
	for i := 0; i < arenaType.NumField(); i++ {
		field := arenaType.Field(i)
		tag := field.Tag.Get("json")
		name := strings.SplitN(tag, ",", 2)[0]
		arenaUnlocks[name] = field.Name
	}
	return arenaUnlocks
}

var ArenaUnlocksFieldByKey = createArenaUnlocksFieldByKey()

func ValidateUnlocks(unlocks *ArenaUnlocks) (*ArenaUnlocks, error) {
	err := validate.Struct(unlocks)
	if err == nil {
		return unlocks, nil
	}
	for _, err := range err.(validator.ValidationErrors) {
		// Fix any fields that are invalid.
...
func NewServerProfile() ServerProfile {
	...
	if err != nil {
		fmt.Printf("JSON decode error!")
		return ServerProfile{}
	}

	// Iterate over all the unlocks using reflect
	// and set them to true if they don't have a restricted/blocked validator tag.
	unlocksType := reflect.TypeOf(serverProfile.UnlockedCosmetics.Arena)
	elem := reflect.ValueOf(&serverProfile.UnlockedCosmetics.Arena).Elem()
	for i := 0; i < unlocksType.NumField(); i++ {
		field := unlocksType.Field(i)
	...
	// Do the same for combat
	unlocksType = reflect.TypeOf(serverProfile.UnlockedCosmetics.Combat)
	elem = reflect.ValueOf(&serverProfile.UnlockedCosmetics.Combat).Elem()
	for i := 0; i < unlocksType.NumField(); i++ {
		field := unlocksType.Field(i)
		tag := field.Tag.Get("validate")
		if strings.Contains(tag, "restricted") || strings.Contains(tag, "blocked") {
