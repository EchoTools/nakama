			...
			if state.Mode == evr.ModeArenaPublic && time.Now().After(state.StartTime.Add(time.Second*60)) && state.GameState != nil && !state.GameState.MatchOver {
				for _, p := range presences {
					if mp, ok := state.presenceMap[p.GetSessionId()]; ok {
						...
						}).Debug("Sending early quit event for player.")

						// Send early quit event - all storage/tier/notification logic is handled by the event processor
						event := NewEventEarlyQuit(
							mp.GetUserId(),
							mp.GetSessionId(),
							state.ID,
							mp.DisplayName,
							state.GetGroupID().String(),
							state.Mode,
						)
						if err := SendEvent(ctx, nk, event); err != nil {
							logger.WithField("error", err).Warn("Failed to send early quit event")
						}
					}
				}
			}
