...
func CalculateSmoothedPlayerRankPercentile(ctx context.Context, db *sql.DB, nk runtime.NakamaModule, userID, groupID string, mode evr.Symbol) (float64, error) {
	...
	dampingPercentile, err := RecalculatePlayerRankPercentile(ctx, db, nk, userID, groupID, mode, settings.ResetScheduleDamper, settings.Default, 20, settings.LeaderboardWeights[mode])
	if err != nil {
		return 0.0, fmt.Errorf("failed to get damping percentile: %w", err)
	}

	activePercentile, err := RecalculatePlayerRankPercentile(ctx, db, nk, userID, groupID, mode, settings.ResetSchedule, dampingPercentile, 3, settings.LeaderboardWeights[mode])
	if err != nil {
...
func RecalculatePlayerRankPercentile(ctx context.Context, db *sql.DB, nk runtime.NakamaModule, userID, groupID string, mode evr.Symbol, resetSchedule evr.ResetSchedule, defaultRankPercentile float64, recordCountMin int, boardNameWeights map[string]float64) (float64, error) {
