...
func MigrateAllUsers(ctx context.Context, logger runtime.Logger, nk runtime.NakamaModule, db *sql.DB, migrations []UserMigrater) error {
	...
	for _, userID := range userIDs {
		startTime := time.Now()
		if err := MigrateUser(ctx, logger, nk, db, userID, migrations); err != nil {
...
func MigrateUser(ctx context.Context, logger runtime.Logger, nk runtime.NakamaModule, db *sql.DB, userID string, migrations []UserMigrater) error {
	startTime := time.Now()
