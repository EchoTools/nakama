...
func TestLoginHistoryEntryKey(t *testing.T) {
	xpid := evr.XPID{PlatformCode: evr.OVR, AccountId: 12345}
	ip := "192.168.1.1"
...
func TestLoginHistoryEntry_Key(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			entry: &LoginHistoryEntry{
				XPID:     evr.XPID{PlatformCode: evr.STM, AccountId: 67890},
				ClientIP: "",
			},
			want: "STM-67890:",
		},
...
func TestLoginHistory_MarshalJSON_SizeLimit(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		entry := &LoginHistoryEntry{
			CreatedAt: time.Now(),
			UpdatedAt: time.Now(),
			XPID:      evr.XPID{PlatformCode: evr.OVR, AccountId: uint64(i)},
