...
type EventRemoteLogSet struct {
	...
	XPID         evr.XPID          `json:"xp_id"`
	Username     string            `json:"username"`
	RemoteLogSet *evr.RemoteLogSet `json:"remote_log_set"`
}

func NewEventPostMatchRemoteLog(userID, sessionID string, xpid evr.XPID, username string, node string, logSet *evr.RemoteLogSet) *EventRemoteLogSet {
...
func (s *EventRemoteLogSet) Process(ctx context.Context, logger runtime.Logger, dispatcher *EventDispatcher) error {
	...
	updates := server.MapOf[uuid.UUID, *MatchGameStateUpdate]{}
	...
	for _, e := range entries {
		...
		case *evr.RemoteLogServerConnectionFailed:
			...
			messageContent := struct {
				...
				ClientEvrID      evr.XPID   `json:"client_evr_id"`
...
func (s *EventRemoteLogSet) incrementCompletedMatches(ctx context.Context, logger runtime.Logger, nk runtime.NakamaModule, sessionRegistry server.SessionRegistry, userID, sessionID string) error {
	// Decrease the early quitter count for the player
	eqconfig := NewEarlyQuitConfig()
	if err := StorableReadNk(ctx, nk, userID, eqconfig, true); err != nil {
		logger.WithField("error", err).Warn("Failed to load early quitter config")
	} else {
		eqconfig.IncrementCompletedMatches()
		if err := StorableWriteNk(ctx, nk, userID, eqconfig); err != nil {
...
func (s *EventRemoteLogSet) processPostMatchTypeStats(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, sessionRegistry server.SessionRegistry, statisticsQueue *StatisticsQueue, msg *evr.RemoteLogPostMatchTypeStats) error {
	var err error
	matchID, err := NewMatchID(msg.SessionUUID(), s.Node)
	if err != nil {
	...
	if playerInfo.Role != BlueTeam && playerInfo.Role != OrangeTeam {
	...
	logger = logger.WithFields(map[string]any{
		...
		"player_xpid": playerInfo.XPID.String(),
	})
	...
	if serviceSettings.UseSkillBasedMatchmaking() && slices.Contains(validModes, label.Mode) {
		...
		blueWins := playerInfo.Role == BlueTeam && msg.IsWinner()
		...
		if rankPercentile, err := CalculateSmoothedPlayerRankPercentile(ctx, db, nk, playerInfo.UserID, groupIDStr, label.Mode); err != nil {
	...
	entries, err := typeStatsToScoreMap(playerInfo.UserID, playerInfo.DisplayName, groupIDStr, label.Mode, msg.Stats)
	if err != nil {
		return fmt.Errorf("failed to convert type stats to score map: %w", err)
	}

	if err := WriteStatistics(ctx, nk, logger, entries); err != nil {
