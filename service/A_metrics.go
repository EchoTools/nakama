...
var _ = server.Metrics(&EVRMetrics{})

type EVRMetrics struct {
	server.Metrics
}

func NewEVRMetrics(metrics server.Metrics) *EVRMetrics {
	return &EVRMetrics{
		Metrics: metrics,
	}
}
...
func ListMatchStates(ctx context.Context, nk runtime.NakamaModule, matchRegistry server.MatchRegistry, query string) ([]*MatchLabelMeta, error) {
	if query == "" {
		query = "*"
	}
	...
	for _, match := range matches {
		...
		presences, tickRate, data, err := matchRegistry.GetState(ctx, mt.UUID, mt.Node)
		if err != nil {
			return nil, err
		}
...
func metricsUpdateLoop(ctx context.Context, logger runtime.Logger, nk runtime.NakamaModule, db *sql.DB, matchRegistry server.MatchRegistry) {
	...
	for {
		...
		matchStates, err := ListMatchStates(ctx, nk, matchRegistry, "")
		if err != nil {
		...
		for tags, count := range playerData {
			...
			tagMap := tags.AsMap()
			nk.MetricsGaugeSet("player_geodata_gauge", tagMap, float64(count))
		}
		...
		for tags := range previouslySeenPlayers {
			if _, ok := seenPlayers[tags]; !ok {
				tagMap := tags.AsMap()
				nk.MetricsGaugeSet("player_geodata_gauge", tagMap, 0)
			}
		}
		...
		for tags, matchCounts := range matchPlayerCounts {
			...
			nk.MetricsGaugeSet("match_active_gauge", tagMap, float64(len(matchCounts)))
			nk.MetricsGaugeSet("player_active_gauge", tagMap, float64(playerCount))
			nk.MetricsGaugeSet("match_rank_percentile_average_variance", tagMap, stat.Mean(percentileVariances[tags], nil))
			nk.MetricsGaugeSet("match_rank_percentile_average", tagMap, stat.Mean(matchRankPercentiles[tags], nil))
		}
		...
		for tags := range previouslySeenMatches {
			if _, ok := seenMatches[tags]; !ok {
				tagMap := tags.AsMap()
				nk.MetricsGaugeSet("match_active_gauge", tagMap, 0)
				nk.MetricsGaugeSet("player_active_gauge", tagMap, 0)
				nk.MetricsGaugeSet("match_rank_percentile_average_variance", tagMap, 0)
				nk.MetricsGaugeSet("match_rank_percentile_average", tagMap, 0)
			}
		}
		...
		if linkedUsers, err := CountLinkedUsers(ctx, nk, db); err != nil {
			...
			nk.MetricsGaugeSet("linked_users_count_gauge", nil, float64(linkedUsers))
		}
		...
		for groupID := range groupIDs {
			...
			for tags, count := range matchmakingPlayers {
				...
				tagMap := tags.AsMap()
				nk.MetricsGaugeSet("matchmaking_active_gauge", tagMap, float64(count))
			}
		...
		for tags := range previouslySeenMatchmaking {
			if _, ok := seenMatchmaking[tags]; !ok {
				tagMap := tags.AsMap()
				nk.MetricsGaugeSet("matchmaking_active_gauge", tagMap, 0)
			}
		}
...
func CountLinkedUsers(ctx context.Context, nk runtime.NakamaModule, db *sql.DB) (int, error) {
	...
	FROM user_device 
	WHERE id LIKE 'OVR-%' OR id LIKE 'DMO-%';
	`
