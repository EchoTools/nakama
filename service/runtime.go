...
func NewNEVRRuntime(logger *zap.Logger, startupLogger *zap.Logger, db *sql.DB, protojsonMarshaler *protojson.MarshalOptions, protojsonUnmarshaler *protojson.UnmarshalOptions, config server.Config, version string, socialClient *social.Client, storageIndex server.StorageIndex, leaderboardCache server.LeaderboardCache, leaderboardRankCache server.LeaderboardRankCache, leaderboardScheduler server.LeaderboardScheduler, sessionRegistry server.SessionRegistry, sessionCache server.SessionCache, statusRegistry server.StatusRegistry, matchRegistry server.MatchRegistry, matchmaker server.Matchmaker, partyRegistry server.PartyRegistry, tracker server.Tracker, router server.MessageRouter, streamManager server.StreamManager, metrics server.Metrics, nkruntime *server.Runtime, runtimeInfo *server.RuntimeInfo, nkPipeline *server.Pipeline, pipeline *Pipeline) (*Runtime, error) {
	...
	if appBotToken, ok := vars["DISCORD_BOT_TOKEN"]; !ok || appBotToken == "" {
		...
		if dg, err = discordgo.New("Bot " + appBotToken); err != nil {
			...
			return nil, fmt.Errorf("unable to create discord bot: %w", err)
		}
	...
	if err := nk.StorageDelete(ctx, []*runtime.StorageDelete{{
		...
		return nil, fmt.Errorf("unable to delete LinkTickets: %w", err)
	}

	// Create the core groups
	if err := createCoreGroups(ctx, db, nk); err != nil {
		return nil, fmt.Errorf("unable to create core groups: %w", err)
	}
	...
	go func() {
		...
		metricsUpdateLoop(ctx, runtimeLogger, nk, db, matchRegistry)
	}()

	/*
		fn := NewAppAPIAcceptor(ctx, runtimeLogger, db, nk, config)
		// Huge hack
		appAcceptorPtr.Store(&fn)

		// Start a websocket listener on port 7355 for EVR
		handler := NewSocketWSEVRAcceptor(logger, config, sessionRegistry, sessionCache, statusRegistry, matchmaker, tracker, metrics, nkruntime, protojsonMarshaler, protojsonUnmarshaler, pipeline)
		server := &http.Server{
			Addr:    ":7355",
			Handler: http.HandlerFunc(handler),
		}
		go func() {
			logger.Info("Starting EVR WebSocket server on :7355")
			if err := server.ListenAndServe(); err != nil && err != http.ErrServerClosed {
				logger.Error("EVR WebSocket server error", zap.Error(err))
			}
		}()
	*/
	return nil, nil
