...
type DiscordIntegrator struct {
	...
	queueCooldowns     *server.MapOf[QueueEntry, time.Time]
	idcache            *server.MapOf[string, string]
}

func NewDiscordIntegrator(ctx context.Context, logger *zap.Logger, config server.Config, metrics server.Metrics, nk runtime.NakamaModule, db *sql.DB, dg *discordgo.Session, guildGroupRegistry *GuildGroupRegistry) *DiscordIntegrator {
...
func (c *DiscordIntegrator) Start() {
	...
	go func() {
		...
		var (
			...
			runtimeLogger  = server.NewRuntimeGoLogger(c.logger)
		)
...
func (c *DiscordIntegrator) syncMember(ctx context.Context, logger *zap.Logger, discordID, guildID string, full bool) error {
	...
	if group.RoleCacheUpdate(profile, member.Roles) {
		if err := GuildGroupStore(ctx, c.db, c.nk, c.guildGroupRegistry, group); err != nil {
...
func (d *DiscordIntegrator) guildSync(ctx context.Context, logger *zap.Logger, guild *discordgo.Guild) error {
	...
	if ownerUserID == "" {
		...
		ownerUserID, _, _, err = server.AuthenticateCustom(ctx, logger, d.db, ownerMember.User.ID, ownerMember.User.Username, true)
		if err != nil {
	...
	if err := GuildGroupStore(ctx, d.db, d.nk, d.guildGroupRegistry, gg); err != nil {
...
func (d *DiscordIntegrator) handleMemberUpdate(logger *zap.Logger, s *discordgo.Session, e *discordgo.GuildMemberUpdate) error {
	...
	if updated := group.RoleCacheUpdate(evrAccount, e.Member.Roles); updated {
		if err := GuildGroupStore(ctx, d.db, d.nk, d.guildGroupRegistry, group); err != nil {
			return fmt.Errorf("error storing guild group: %w", err)
		}
	}
