...
var (
	...
	ErrJoinRejectDuplicateXPID                   = errors.New("duplicate XP ID")
...
func (s *MatchLabel) GetPlayerCount() int {
	...
	for _, p := range s.Players {
		if int(p.Role) != evr.TeamSpectator && int(p.Role) != evr.TeamModerator {
			count++
		}
	}
	return count
...
func (s *MatchLabel) GetPlayerByEvrID(evrID evr.XPID) *PlayerInfo {
	for _, p := range s.Players {
		if p.XPID == evrID {
			return &p
		}
...
func (s *MatchLabel) OpenSlotsByRole(role RoleIndex) (int, error) {
...
func (s *MatchLabel) roleLimit(role RoleIndex) int {
...
func (s *MatchLabel) RoleCount(role RoleIndex) int {
	count := 0
	for _, p := range s.Players {
		if p.Role == role {
			count++
