...
func NewInGamePanel(nk runtime.NakamaModule, logger runtime.Logger, sessionRegistry server.SessionRegistry, cache *DiscordIntegrator, dg *discordgo.Session, userID string) *InGamePanel {
...
func (p *InGamePanel) retrieveActiveLoginSession(userID string) (server.Session, error) {
	...
	for _, presence := range presences {
		if presence.GetUserId() == userID {
			...
			return p.sessionRegistry.Get(sessionID), nil
		}
	}
...
func (p *InGamePanel) createUpdateEdit(guildID string, label *MatchLabel, recentPlayers []PlayerInfo) *discordgo.WebhookEdit {
	...
	for _, p := range label.Players {
		...
		playerNames = append(playerNames, discordgo.SelectMenuOption{
			Label:       p.DisplayName,
			Value:       p.UserID,
			Description: fmt.Sprintf("%s / %s", p.Username, p.XPID.String()),
...
func (d *DiscordAppBot) handleInGamePanel(ctx context.Context, logger runtime.Logger, s *discordgo.Session, i *discordgo.InteractionCreate, user *discordgo.User, member *discordgo.Member, userID string, groupID string) error {
	...
	igp = NewInGamePanel(d.nk, logger, d.sessionRegistry, d.cache, s, userID)
