	...
	for {
		...
		if len(matches) != 0 {
			...
			slices.SortStableFunc(labels, func(a, b *MatchLabel) int {
				if a.RoleCount(Spectator) == 0 && b.RoleCount(Spectator) > 0 {
					return -1
				} else if a.RoleCount(Spectator) > 0 && b.RoleCount(Spectator) == 0 {
					return 1
				}
				return int(a.Size - b.Size)
			})
			...
			for _, label := range labels {
				entrant, err := EntrantPresenceFromSession(session, uuid.Nil, Spectator, types.Rating{}, 0, label.GetGroupID().String(), 0, "")
				if err != nil {
					logger.Warn("Failed to create entrant presence", zap.String("session_id", session.ID().String()), zap.Error(err))
					continue
				}

				entrant.RoleAlignment = Spectator
