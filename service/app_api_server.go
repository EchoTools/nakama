...
func NewAppAPI(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule) *AppAPI {
	return &AppAPI{
		ctx:    ctx,
		logger: logger,
		db:     db,
...
type EVRAPIServer struct {
	server.ApiServer
}

func NewAppAPIAcceptor(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, config server.Config) func(http.ResponseWriter, *http.Request) {
	appAPI := NewAppAPI(ctx, logger, db, nk)

	signingKey := config.GetSession().EncryptionKey
	node := config.GetName()
