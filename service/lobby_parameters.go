...
type LobbySessionParameters struct {
	...
	Role                         RoleIndex                     `json:"role"`
	PartySize                    *atomic.Int64                 `json:"party_size"`
	PartyID                      uuid.UUID                     `json:"party_id"`
	PartySharedKey               string                        `json:"party_group_name"`
...
func (s LobbySessionParameters) MetricsTags() map[string]string {
	return map[string]string{
		...
		"role":             strconv.Itoa(int(s.Role)),
	}
}

func NewLobbyParametersFromRequest(ctx context.Context, logger *zap.Logger, nk runtime.NakamaModule, session *sessionEVR, request evr.LobbySessionRequest, node string) (*LobbySessionParameters, error) {
	...
	entrantRole := RoleIndex(request.GetEntrantRole(0))
	...
	if !userSettings.NextMatchID.IsNil() {
		...
		if label, err := MatchLabelByID(ctx, nk, userSettings.NextMatchID); err != nil {
			...
			if userSettings.NextMatchRole != "" {
				...
				case "orange":
					entrantRole = OrangeTeam
				case "blue":
					entrantRole = BlueTeam
				case "spectator":
					entrantRole = Spectator
				case "moderator":
					entrantRole = Moderator
				case "any":
					entrantRole = AnyTeam
				}
			}
		...
		go func() {
			...
			if err := StorableWriteNk(ctx, p.nk, userID, userSettings); err != nil {
	...
	if (mode == evr.ModeArenaPublic || mode == evr.ModeCombatPublic) && globalSettings.EnableSBMM && groupID != uuid.Nil {
		...
		if userSettings.StaticBaseRankPercentile > 0 {
			...
			rankPercentile, err = CalculateSmoothedPlayerRankPercentile(ctx, p.db, p.nk, userID, groupIDStr, mmMode)
			if err != nil {
...
func (p *LobbySessionParameters) FromMatchmakerEntry(entry *server.MatchmakerEntry) {
...
func (p LobbySessionParameters) MatchmakingStream() server.PresenceStream {
	return server.PresenceStream{Mode: StreamModeMatchmaking, Subject: p.GroupID}
}

func (p LobbySessionParameters) GuildGroupStream() server.PresenceStream {
	return server.PresenceStream{Mode: StreamModeGuildGroup, Subject: p.GroupID, Label: p.Mode.String()}
}

func (p LobbySessionParameters) PresenceMeta() server.PresenceMeta {
	return server.PresenceMeta{
		Status: p.String(),
	}
}
