...
func NewLobbyBuilder(logger runtime.Logger, nk runtime.NakamaModule, sessionRegistry server.SessionRegistry, matchRegistry server.MatchRegistry, tracker server.Tracker, metrics server.Metrics) *LobbyBuilder {
	logger = logger.WithField("module", "lobby_builder")
...
func (b *LobbyBuilder) rankEndpointsByAverageLatency(entrants []*server.MatchmakerEntry) (map[string]int, map[string]map[string]float64) {
...
// SortGameServerIPs sorts the game server IPs by latency, returning a slice of external IP addresses
func (b *LobbyBuilder) rankEndpointsByServerScore(entrants []*server.MatchmakerEntry) []string {
...
func (b *LobbyBuilder) groupByTicket(entrants []*server.MatchmakerEntry) [][]*server.MatchmakerEntry {
	partyMap := make(map[string][]*server.MatchmakerEntry, 8)
	for _, e := range entrants {
		t := e.GetTicket()
		partyMap[t] = append(partyMap[t], e)
	}

	parties := make([][]*server.MatchmakerEntry, 0, len(partyMap))
...
func (b *LobbyBuilder) buildMatch(logger runtime.Logger, entrants []*server.MatchmakerEntry) (matchID *MatchID, err error) {
	// Build matches one at a time.

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	logger = logger.WithField("entrants", len(entrants))
	...
	teams := [2][]*server.MatchmakerEntry{}
	...
	entrantPresences := make([]*LobbyPresence, 0, len(entrants))
	...
	for teamIndex, players := range teams {
		...
		for _, entry := range players {
			...
			if entrant, err := EntrantPresenceFromSession(session, MatchIDFromStringOrNil(entry.GetPartyId()).UUID, RoleIndex(teamIndex), rating, percentile, groupID.String(), 0, query); err != nil {
	...
	successful := make([]*LobbyPresence, 0, len(entrants))
	errored := make([]*LobbyPresence, 0, len(entrants))
	wg := &sync.WaitGroup{}
	wg.Add(len(entrantPresences))

	erroredCh := make(chan *LobbyPresence, len(entrantPresences))
	succeededCh := make(chan *LobbyPresence, len(entrantPresences))

	for i, p := range entrantPresences {
		go func(session server.Session, p *LobbyPresence) {
			defer wg.Done()
			if p == nil {
				return
			}
...
func (b *LobbyBuilder) groupIDFromEntrants(entrants []*server.MatchmakerEntry) (uuid.UUID, error) {
...
func (b *LobbyBuilder) distributeParties(parties [][]*server.MatchmakerEntry) [][]*server.MatchmakerEntry {
...
func rttByPlayerByExtIP(ctx context.Context, logger *zap.Logger, db *sql.DB, nk runtime.NakamaModule, groupID string) (map[string]map[string]int, error) {
	...
	for _, label := range pubLabels {
		for _, p := range label.Players {
			...
			if err := StorableReadNk(ctx, nk, p.UserID, history, true); err != nil && status.Code(err) != codes.NotFound {
