...
func NewSessionEVR(logger *zap.Logger, config server.Config, format server.SessionFormat, sessionID, userID uuid.UUID, username string, vars map[string]string, expiry int64, clientIP, clientPort, lang string, protojsonMarshaler *protojson.MarshalOptions, protojsonUnmarshaler *protojson.UnmarshalOptions, conn *websocket.Conn, sessionRegistry server.SessionRegistry, statusRegistry server.StatusRegistry, matchmaker server.Matchmaker, tracker server.Tracker, metrics server.Metrics, pipeline *server.Pipeline, evrPipeline *EvrPipeline, runtime *server.Runtime, request http.Request, storageIndex server.StorageIndex) server.Session {
	...
	params := SessionParameters{
		...
		party:                atomic.NewPointer[*api.Party](nil),
	...
	if format == server.SessionFormatProtobuf || format == SessionFormatEVR {
	...
	if params.authDiscordID != "" {
		...
		if userIDStr := evrPipeline.discordCache.DiscordIDToUserID(params.authDiscordID); userIDStr == "" {
			...
			account, err := server.GetAccount(ctx, logger, evrPipeline.db, statusRegistry, uuid.FromStringOrNil(userIDStr))
			if err != nil {
				logger.Warn("Failed to get account by Discord ID", zap.Error(err))
			} else if account == nil {
				logger.Warn("Account not found by Discord ID")
			} else {
				userIDStr, err := server.AuthenticateUsername(ctx, logger, evrPipeline.db, account.User.Username, passwd)
				if err != nil {
...
IncomingLoop:
	for {
		...
		if s.format == SessionFormatEVR {
			...
			switch s.format {
			case server.SessionFormatProtobuf:
				err = proto.Unmarshal(data, request)
			case server.SessionFormatJson:
				fallthrough
			default:
...
func (s *sessionEVR) maybeResetPingTimer() bool {
...
func (s *sessionEVR) pingNow() (string, bool) {
	s.Lock()
	if s.stopped {
		s.Unlock()
		return "", false
...
func (s *sessionEVR) SendEvrUnrequire(messages ...evr.Message) error {
...
func (s *sessionEVR) SendEvr(messages ...evr.Message) error {
...
func (s *sessionEVR) SendBytes(payload []byte, reliable bool) error {
	...
	default:
		...
		go s.Close(server.ErrSessionQueueFull.Error(), runtime.PresenceReasonDisconnect)
