...
func (h *RPCHandler) AuthenticatePasswordRPC(ctx context.Context, logger runtime.Logger, db *sql.DB, _nk runtime.NakamaModule, payload string) (string, error) {
	...
	token, exp := generateToken(h.config, tokenID, tokenIssuedAt, userID, username, vars)
	refreshToken, refreshExp := generateRefreshToken(h.config, tokenID, tokenIssuedAt, userID, username, vars)
