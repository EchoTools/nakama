...
type PlayerInfo struct {
	...
	Role           RoleIndex `json:"team"`
	JoinTime       time.Time `json:"join_time_ms,omitempty"` // The time on the round clock that the player joined
...
func (p *PlayerInfo) IsCompetitor() bool {
	return p.Role == BlueTeam || p.Role == OrangeTeam
}
