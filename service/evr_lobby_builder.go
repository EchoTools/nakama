...
func NewLobbyBuilder(logger *zap.Logger, nk runtime.NakamaModule, sessionRegistry server.SessionRegistry, matchRegistry server.MatchRegistry, tracker server.Tracker, metrics server.Metrics) *LobbyBuilder {
...
func (b *LobbyBuilder) buildMatch(logger *zap.Logger, entrants []*server.MatchmakerEntry) (matchID *MatchID, err error) {
	// Build matches one at a time.
	...
	for {
		...
		label, err = LobbyGameServerAllocate(ctx, server.NewRuntimeGoLogger(logger), b.nk, []string{groupID.String()}, meanRTTByExtIP, settings, nil, true, false, queryAddon)
