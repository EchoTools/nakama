...
func (p *Pipeline) JoinMatchmakingStream(logger *zap.Logger, s *sessionEVR, lobbyParams *LobbySessionParameters) error {
	...
	p.router.SendToStream(logger, stream, &rtapi.Envelope{
		Message: &rtapi.Envelope_StreamData{
			StreamData: &rtapi.StreamData{
				Stream: &rtapi.Stream{
					Mode:    int32(server.SessionFormatJson),
...
func LeaveMatchmakingStream(logger *zap.Logger, s *sessionEVR) error {
	s.tracker.UntrackLocalByModes(s.id, map[uint8]struct{}{StreamModeMatchmaking: {}}, server.PresenceStream{})
	return nil
}
