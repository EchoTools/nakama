...
type EVRProfile struct {
	...
	GhostedPlayers         []evr.XPID                 `json:"ghosted_players"`           // The ghosted players
...
func (e EVRProfile) XPIDs() []evr.XPID {
	xpids := make([]evr.XPID, 0, len(e.account.Devices))
	for _, d := range e.account.Devices {
...
func (e EVRProfile) LinkedXPIDs() []evr.XPID {
	devices := make([]evr.XPID, 0, len(e.account.Devices))
	for _, d := range e.account.Devices {
...
func (a EVRProfile) GetMuted() []evr.XPID {
	if a.MutedPlayers == nil {
		return make([]evr.XPID, 0)
	}
	return a.MutedPlayers
}

func (a EVRProfile) GetGhosted() []evr.XPID {
