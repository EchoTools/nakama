...
import (
	...
	"github.com/echotools/nakama/backend"
...
func InitModule(ctx context.Context, runtimeLogger runtime.Logger, db *sql.DB, nk runtime.NakamaModule, initializer runtime.Initializer) error {
	...
	d := ctx.Value(service.PipelineDependenciesKey{}).(*service.PipelineDependencies)
	logger := d.Logger

	if _, err := service.ServiceSettingsLoad(ctx, nk); err != nil {
		runtimeLogger.WithField("err", err).Error("Failed to load global settings")
		panic(err)
	}

	backend.InitializeHooks(initializer)

	// Create the core groups
	if err := service.CreateCoreGroups(ctx, db, nk); err != nil {
	...
	if err := initializer.RegisterMatch(service.EVRLobbySessionMatchModule, func(ctx context.Context, logger runtime.Logger, db *sql.DB, nk runtime.NakamaModule) (runtime.Match, error) {
		return &backend.NEVRMatch{}, nil
	}); err != nil {
		return err
	}

	userRemoteLogJournalRegistry := service.NewUserRemoteLogJournalRegistry(ctx, d.Logger, nk, d.SessionRegistry)
	...
	discordBotID := service.ServiceSettings().DiscordBotUserID
	guildGroupRegistry := service.NewGuildGroupRegistry(ctx, runtimeLogger, nk, db, discordBotID)

	var statisticsQueue *service.StatisticsQueue
	var vrmlScanQueue *service.VRMLScanQueue
	if vars["VRML_REDIS_URI"] == "" || vars["VRML_OAUTH_REDIRECT_URL"] == "" || vars["VRML_OAUTH_CLIENT_ID"] == "" {
		...
		redisClient, err := service.ConnectRedis(ctx, redisURI)
		if err != nil {
			return fmt.Errorf("failed to connect to Redis for VRML scan queue: %w", err)
		}
		vrmlScanQueue, err = service.NewVRMLScanQueue(ctx, runtimeLogger, db, nk, initializer, dg, redisClient, vrmlOAuthRedirectURL, vrmlOAuthClientID)
		if err != nil {
	...
	redisClient, err := service.ConnectRedis(ctx, vars["REDIS_URI"])
	if err != nil {
		d.Logger.Fatal("Failed to connect to Redis", zap.Error(err))
	}

	// TODO: Move component creation to main.go
	var ipInfoCache *service.IPInfoCache
	if redisClient != nil {
		var providers []service.IPInfoProvider
		if vars["IPAPI_API_KEY"] != "" {
			ipqsClient, err := service.NewIPQSClient(logger, d.Metrics, redisClient, vars["IPQS_API_KEY"])
			if err != nil {
		...
		ipapiClient, err := service.NewIPAPIClient(logger, d.Metrics, redisClient)
		if err != nil {
			logger.Fatal("Failed to create IPAPI client", zap.Error(err))
		}
		providers = append(providers, ipapiClient)

		ipInfoCache, err = service.NewIPInfoCache(logger, d.Metrics, providers...)
		if err != nil {
	...
	if mongoURI := vars["MONGODB_URI"]; mongoURI != "" {
		if mongoClient, err = service.ConnectMongoDB(ctx, mongoURI); err != nil {
	...
	eventDispatch, err := service.NewEventDispatch(ctx, runtimeLogger, db, nk, d.SessionRegistry, d.MatchRegistry, mongoClient, redisClient, dg, statisticsQueue, vrmlScanQueue)
	if err != nil {
	...
	// TODO: Move component creation to main.go
	discordIntegrator := service.NewDiscordIntegrator(ctx, logger, d.Config, d.Metrics, nk, db, dg, guildGroupRegistry)
	discordIntegrator.Start()

	// TODO: Move component creation to main.go
	appBot, err := service.NewDiscordAppBot(ctx, runtimeLogger, nk, db, d.Metrics, d.Config, discordIntegrator, d.StatusRegistry, d.SessionRegistry, dg, ipInfoCache, guildGroupRegistry)
	if err != nil {
		logger.Error("Failed to create app bot", zap.Error(err))

	}
	// Store the app bot in a global variable for later use
	service.GlobalAppBot.Store(appBot) // FIXME: TODO: This is a temporary solution, we should refactor this to avoid global state
	...
	evrPipeline := service.NewEvrPipeline(
		d.Logger,
		d.StartupLogger,
		db,
		nk,
	...
	if err := initializer.RegisterHttp("/evr", service.NewSocketWSEVRAcceptor(d.Logger, d.Config, d.SessionRegistry, d.SessionCache, d.StatusRegistry, d.Matchmaker, d.Tracker, d.Metrics, d.ProtojsonMarshaler, d.ProtojsonUnmarshaler, evrPipeline), http.MethodGet); err != nil {
	...
	lobbyBuilder := service.NewLobbyBuilder(runtimeLogger, nk, d.SessionRegistry, d.MatchRegistry, d.Tracker, d.Metrics)
	...
	go service.MigrateSystem(ctx, runtimeLogger, db, nk)

	// Start the service settings loop
	go service.ServiceSettingsLoop(ctx, runtimeLogger, nk)

	// Update the metrics with match data
	go func() {
		<-time.After(15 * time.Second)
		service.MetricsUpdateLoop(ctx, runtimeLogger, nk, db, d.MatchRegistry)
	}()
	return nil
}
